...
void Scene::DrawPlayerHitsUI()
{
	...
	Engine::GetInstance().render.get()->DrawTexture(
		...
		&section // Secci�n de la textura a dibujar
	);
}
	...
	for (int i = 0; i < MAX_PICKAXES; ++i) {
		...
		// Si no, dibuja una piqueta gastada
			...
			Engine::GetInstance().render.get()->DrawTexture(
				...
				-Engine::GetInstance().render.get()->camera.y / Engine::GetInstance().window.get()->scale + 80    // Posici�n Y
			);
...
void Scene::DrawCurrencyUI()
{
	...
	// Tama�o de la secci�n de la textura del soul orb
	SDL_Rect section;
	section.x = 0;  // Posici�n X dentro de la textura
	section.y = 128; // Posici�n Y dentro de la textura
	section.w = 64; // Ancho de la secci�n
	section.h = 96; // Alto de la secci�n
